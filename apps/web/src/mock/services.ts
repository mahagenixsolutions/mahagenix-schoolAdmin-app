...
const getStudent = (studentId?: string) =>
  mockStudents.find((student) => student.id === studentId) || mockStudents[0];
...
const buildClassDetails = (grade: string, params: Params) => {
  const cls =
    mockClasses.find((item) => item.grade === grade || item.id === grade) || mockClasses[0];
  ...
  if (search) {
    roster = roster.filter((student) =>
      [student.full_name, student.student_code, student.admission_no].some((value) =>
        value.toLowerCase().includes(search),
      ),
    );
  }
  if (section && section !== 'all')
    roster = roster.filter((student) => student.section === section);
  ...
  const avgMarks = Math.round(
    fullRoster.reduce((sum, student) => sum + student.average_marks, 0) /
      Math.max(1, fullRoster.length),
  );
  const topStudent = [...fullRoster].sort((a, b) => b.average_marks - a.average_marks)[0];
  const atRisk = fullRoster.filter(
    (student) => student.attendance_rate < 88 || student.average_marks < 74,
  );
  ...
  return {
    ...
    students: {
      ...
      meta: {
        ...
        page,
        limit,
        total_pages: Math.max(1, Math.ceil(roster.length / limit)),
      },
    },
    analytics: {
      attendance_trend: mockAttendanceTrend.map((item, index) => ({
        month: item.month,
        rate: Math.max(72, toNumberPercent(cls.attendance) - 4 + (index % 5)),
      })),
      marks_trend: ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun'].map((month, index) => ({
        month,
        average: Math.max(55, avgMarks - 5 + index * 2),
      })),
      subject_performance: mockSubjects
        ...
        .map((subject) => ({
          ...
          average: Math.round(
            fullRoster.reduce((sum, student) => sum + getMockScore(student.id, subject.id), 0) /
              Math.max(1, fullRoster.length),
          ),
        })),
...
const buildSubjectDashboard = (subjectId: string) => {
  ...
  const eligibleClasses = mockClasses.filter(
    (cls) => !subject.classes || subject.classes.includes(cls.grade),
  );
  const eligibleStudents = mockStudents.filter((student) =>
    eligibleClasses.some((cls) => cls.id === student.class_id),
  );
    ...
    classMapping: eligibleClasses.map((cls) => {
      ...
      return {
        ...
        averageMarks: Math.round(
          roster.reduce((sum, student) => sum + getMockScore(student.id, subject.id), 0) /
...
const buildAcademicYearDetails = (id: string) => {
  ...
  return {
    ...
    calendar: [
      {
        id: 'event-1',
        title: 'Academic Year Opening Day',
    ...
    achievements: [
      {
        ...
        description: 'Senior students won first place for a renewable-energy model.',
        level: 'INTERSCHOOL',
        category: 'ACADEMIC',
      },
      {
        id: 'ay-ach-2',
        title: 'District Athletics Finalists',
        description: 'Students qualified for district track and field finals.',
        level: 'DISTRICT',
        category: 'SPORTS',
      },
      {
        ...
        description: 'Classes 1-5 completed a 1,000-book reading challenge.',
        level: 'SCHOOL',
        category: 'CULTURAL',
      },
    ],
...
const getStudentBundle = (studentId?: string) => {
  ...
  return {
    ...
    marks_summary: mockSubjectAverages.map((item) => ({
    ...
    remarks: [
      {
        id: 'remark-1',
        category: 'Academic',
        remark: 'Shows strong curiosity and asks thoughtful questions.',
...
export function resolveMockRequest(input: any) {
  const request =
    typeof input === 'string'
      ? { url: input, method: 'GET', params: undefined, body: undefined }
      : input;
  ...
  if (method !== 'GET')
    return mutationOk({ ...request.body, id: `mock-${Date.now()}`, saved: true });
  ...
  if (path.startsWith('marks/class/'))
    return ok(getMarksRoster(path.split('/')[2], params?.examId, params?.subjectId));
    ...
    return ok([
      {
        ...
        topics: dashboard.classMapping.slice(0, 4).map((cls: any, index: number) => ({
          ...
          topic_name: ['Core concepts', 'Vocabulary and symbols', 'Guided practice', 'Class quiz'][
            index
          ],
  ...
  if (path.match(/^subjects\/[^/]+\/resources$/)) {
    ...
    return ok([
      {
        id: `${subjectId}-res-1`,
        title: `${subject.name} practice worksheet`,
        type: 'PDF',
        url: '#',
        uploader: teacher,
      },
      {
        id: `${subjectId}-res-2`,
        title: `${subject.name} concept slides`,
        type: 'Slides',
        url: '#',
        uploader: teacher,
      },
      {
        id: `${subjectId}-res-3`,
        title: `${subject.name} revision video`,
        type: 'Video',
        url: '#',
        uploader: teacher,
      },
    ...
    return ok([
      {
        id: `${subjectId}-act-1`,
        teacher,
        action: `updated ${subject.name} syllabus progress`,
        created_at: '2026-06-18T09:30:00Z',
      },
      {
        id: `${subjectId}-act-2`,
        teacher,
        action: 'uploaded a practice worksheet',
        created_at: '2026-06-17T12:15:00Z',
      },
      {
        id: `${subjectId}-act-3`,
        teacher,
        action: 'reviewed quarterly assessment scores',
  ...
  if (path.startsWith('analytics/parent-kpis/'))
    return ok({ attendance: 93, average_marks: 86, homework_completion: 92, badges: 3 });
  if (path === 'analytics/attendance-trend') return ok(mockAttendanceTrend);
  if (path === 'analytics/marks-trend')
    return ok([
      { month: 'Apr', score: 78 },
      { month: 'May', score: 83 },
      { month: 'Jun', score: 87 },
  ...
  if (path === 'dashboard/alerts')
    return ok({
      alerts: [
        {
          severity: 'warning',
          message: 'Class 8 A attendance needs follow-up this week',
          link: '/classes/class-8-a',
        },
        { severity: 'info', message: 'Quarterly marks draft is 84% complete', link: '/marks' },
        {
          severity: 'success',
          message: 'Class 10 A completed all mock exam entries',
          link: '/classes/class-10-a',
        },
      ],
    });
  ...
  if (path === 'dashboard/today-attendance-breakdown')
    return ok([
      { name: 'Present', value: 64 },
      { name: 'Absent', value: 4 },
      { name: 'Late', value: 3 },
      { name: 'Leave', value: 1 },
    ]);
  if (path === 'dashboard/fee-collection-trend')
    return ok([
      { month: 'Jan', collected: 72 },
      { month: 'Feb', collected: 78 },
      { month: 'Mar', collected: 84 },
      { month: 'Apr', collected: 88 },
      { month: 'May', collected: 91 },
      { month: 'Jun', collected: 88 },
    ]);
  ...
  if (path === 'dashboard/insights')
    return ok({
      insights: [
        {
          id: 'insight-1',
          title: 'Class 10 A leads academic performance',
          value: '94%',
          tone: 'success',
          icon: 'ti ti-trophy',
        },
        {
          id: 'insight-2',
          title: 'Class 8 A attendance needs attention',
          value: '89%',
          tone: 'warning',
        ...
        {
          id: 'insight-3',
          title: 'Science scores improved across middle school',
          value: '+6%',
          tone: 'info',
  ...
  if (path === 'progress/analytics')
    return ok({
      completed: 87,
      pending: 13,
      trend: [
        { day: 'Mon', completed: 81 },
        { day: 'Tue', completed: 86 },
        { day: 'Wed', completed: 89 },
      ],
    });
  if (path.startsWith('progress/student/'))
    return ok(getStudentProgressHistory(path.split('/')[2]));

  if (path === 'participation/overview')
    return ok({
      total_points: 186,
      active_students: mockStudents.length,
      achievements: mockAchievements.length,
      badges: mockBadges.length,
    });
  if (path === 'participation/analytics')
    return ok({
      by_category: [
      ...
      awards: [
        {
          ...
          description: null,
          awarded_by: 'EduTrack Board of Education',
          awarded_date: '2023-05-15T00:00:00Z',
        ...
        },
        {
          ...
          description: null,
          awarded_by: 'State EdTech Consortium',
          awarded_date: '2021-11-20T00:00:00Z',
  ...
  if (path === 'parent/attendance')
    return ok({ percentage: 93, present: 21, absent: 1, late: 1, trend: mockAttendanceTrend });
  if (path === 'parent/marks')
    return ok(
      mockSubjectAverages.map((item) => ({
        ...
        score: item.average,
        max_score: 100,
        grade: item.average >= 90 ? 'A' : 'B+',
      })),
    );
  if (path === 'parent/homework')
    return ok([
      { title: 'Reading log', due_date: '2026-06-19', status: 'COMPLETED' },
      { title: 'Math worksheet', due_date: '2026-06-20', status: 'PENDING' },
    ]);
  ...
  if (path === 'parent/timeline')
    return ok([
      { id: 'timeline-1', type: 'attendance', title: 'Marked present', date: '2026-06-18' },
      { id: 'timeline-2', type: 'marks', title: 'Scored 88 in English', date: '2026-06-17' },
    ]);
    ...
    return ok([
      {
        ...
        teacherName: 'Nisha Rao',
        lastMessage: 'Great progress this week.',
        messages: [
          { sender: 'teacher', content: 'Great progress this week.', at: '2026-06-17T10:00:00Z' },
        ],
      },
    ]);
  if (path === 'parent/reports')
    return ok([
      { id: 'report-1', title: 'Quarterly Progress Report', status: 'Ready', date: '2026-06-15' },
    ]);
  ...
  if (path === 'parent/rankings')
    return ok([
      { scope: 'Class', rank: 4, total: 8 },
      { scope: 'Grade', rank: 7, total: 16 },
    ]);
  if (path === 'parent/remarks') return ok(getStudentBundle(params?.studentId).remarks);
  if (path === 'parent/gallery')
    return ok([
      {
        id: 'photo-1',
        title: 'Art showcase',
  ...
  if (path === 'parent/ai-insights')
    return ok({
      summary: 'Consistent engagement with a clear upward trend in reading and number fluency.',
      strengths: ['Reading fluency', 'Peer collaboration'],
      focus: ['Timed arithmetic practice'],
    });
